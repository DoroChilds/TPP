...
For the data manipulations in this vignette, we also load the \Rpackage{dplyr}
and \Rpackage{magrittr} packages:
...
their long-term melting behaviour (constrained by the maximum plateu paramter):
