...
- By default, Treatment groups are compared to Vehicle groups within each replicate. It ist important that only one Treatment and Vehicle group are specified per Replicate (if specified in the condition column).
- If valid comparison columns are specified in the config table, only those comparisons are performed.
