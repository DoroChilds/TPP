...
#' @return None
#' 
#' @details Creates pdf files of the afore created plots by \code{plotSplines},
#' \code{tpp2dPlotCCRAllCurves}, \code{tpp2dPlotCCRGoodCurves}, 
    ...
    if (!is.null(plotList[[pl]]) && !is.na(plotList[[pl]])){
      if (type=="all" | type=="good"){
        savePl <- try(suppressMessages(ggsave(plotList[[pl]], 
                                              filename=file.path(plotPath, paste(gsub("\\|", "_", gsub("(\\.)", "_", pl)),"2D_TPP", 
                                                                                 type, "plots.pdf", sep="_")))))
        if(class(savePl) == "try-error"){
          setwd(plotPath)
          suppressMessages(ggsave(plotList[[pl]], 
                                  filename=paste(gsub("\\|", "_", gsub("(\\.)", "_", pl)),"2D_TPP", 
                                                 type, "plots.pdf", sep="_")))
        }
        ...
        if (!is.null(plotList[[pl]])){
          invisible(lapply(names(plotList[[pl]]), function(temp){
            ...
            savePl <- try(suppressMessages(ggsave(plotList[[pl]][[temp]], 
                                                  filename=file.path(plotPath, paste(gsub("\\|", "_", gsub("(\\.)", "_", pl)), 
                                                                                     gsub("(\\.)", "_", temp), 
