...
\begin{abstract}
This document demonstrates how to analyze TPP-TR (temperature range) experiments by the NPARC approach \cite{childs2019}. NPARC is a recent extension to the \Rpackage{TPP} package. It offers a novel methodology to model the temperature dependent melting behaviour of each protein, and to detect significant changes in this behavior due to changes in experimental conditions like drug treatment \cite{savitski2014tracking}.
...
\section{Using NPARC }
The integration of NPARC into the \Rpackage{TPP} is currently ongoing (you can find the GitHub repository \href{https://github.com/DoroChilds/NPARC}{here}). If you want to try it in the mean time please refer to \href{https://git.embl.de/childs/TPP-data-analysis/tree/master/NPARC_paper/reports}{this workflow}.  
...
% <<analyzeTR, eval = TRUE, warning=FALSE>>=
