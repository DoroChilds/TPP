plotSplines <- function(tppData_long_normalized, refTableLong){
  ...
  resultList <- lapply(uniqueIDs, function(id_tmp){
    ...
    if (nrow(refDF_tmp) >= 10 && nrow(tppDF_tmp) >= 30 && length(which(is.na(tppDF_tmp$fcNormalized))) < 10){
      tppDF_tmp$drugConc <- as.factor(tppDF_tmp$drugConc)
      tppDF_tmp$drugConc <- factor(tppDF_tmp$drugConc, 
                                   levels=paste0(sort(gsub("[^0-9,\\.]", "",levels(tppDF_tmp$drugConc))), "uM"))
      
      p <- ggplot(refDF_tmp, aes(x = temperature, y = relConc)) +
        geom_point(aes(shape=condition), color="gray30") +
        stat_smooth(data = refDF_tmp, method = rlm, method.args = list(maxit=150),
                    formula=(y ~ ns(x, df=4)), se=FALSE, color="gray30", linetype=2) +
