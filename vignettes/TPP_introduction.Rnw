...
\date{\Rpackage{TPP} version \Sexpr{packageDescription("TPP")$Version} (Last revision 2015-10-16)}
...
print(onlyBNew)
@
%We observe that decreasing the bin width by \Sexpr{300 - bNew} leads to the detection of \Sexpr{length(onlyBNew)} new target protein\Sexpr{ifelse(length(as.character(onlyBNew))==1, "", "s")}, while \Sexpr{length(as.character(onlyB300))} of the previous targets \Sexpr{ifelse(length(as.character(onlyB300))==1, "is", "are")} now omitted.
